  ...
  #          print('input, value in lattice encoder in transformer', input_norm_split1.size(), value_norm_split.size())#test
     ...
     #   context = torch.cat([self.latt_attn(input_norm_split2, value_norm_split2) for
       ...
       # print("size of input 2", type(input_norm_split2), input_norm_split2)
        ...
        align_sum_all = torch.zeros(1, 2)
        for i in range(len(input_norm_split2s)):
            
            logger.info(i) # get number i
            context_temp, _, align_sum = self.latt_attn(input_norm_split2s[i], value_norm_splits4atts[i], n, latt=True)  
            align_sum_all += align_sum.cpu()
          ...
          #  logger.info("context_gates log")  #test
          #  logger.info(i)
          #  logger.info(context_gates) #test
            context_gates_sum = torch.sum(context_gates)
           # if context_gates_sum + 0.1 < len(context_gates):
           #     logger.info("context_gates log")
           #     logger.info(i)
           #     logger.info(context_gates)
            context_temp = torch.mul(context_temp, context_gates)  # 
            contexts.append(context_temp)
            
        logger.info("align_sum_all")
        logger.info(align_sum_all) #test    
...
# latt included within normal transformer, interleaved, for the first three layers
 ...
 #           print(i, 'out', out) #test
  ...
  #      if feat_merge == 'latt':
  #          out = self.lattice[0](out, out_latt, 0)
